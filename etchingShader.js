...
let etchingShaderInstanced = {
    name: 'EtchingShaderInstanced',
    vertexShader: `
    // uniform vec3 lightDir; // Direction to the light source in camera space
    uniform float tilingFactor; // To control the tiling of the hatching texture
    ...
    void main() {
        vec3 lightDir = vec3(0.0, 0.0, 1.0); // Example light direction
        vec4 modelViewPosition = vec4(position, 1.0);// Transform vertex position to camera space
        ...
        gl_Position = projectionMatrix * modelViewMatrix * modelViewPosition;// Transform vertex position to screen space
    }`,
    fragmentShader: `
    uniform sampler2D texture1; 
    uniform float posCamVsUV;
...
export { etchingShader, etchingShaderInstanced };